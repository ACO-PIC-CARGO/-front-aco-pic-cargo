  ...
  async generarReporte(
    ...
    if (tipo == "AGRUPADO") {
      opcion.listCostos
        ...
        .forEach((element) => {
          let factorCalculado = miMixin.methods.calcularFac(
          ...
          if (element.esgastostercerosflag == 1) {
            ...
            if (
              ...
              totalGastosTercero += operacionFinal;
            } else {
        ...
        .forEach((element) => {
          ...
          if (element.esfleteflag == 1) {
            if (
              ...
              datosFlete.push({
                name: element.nameservice,
                valor: miMixin.methods.currencyFormat(operacionFinal),
              });
              totalFlete += operacionFinal;
            } else {
              datosFlete.push({
            ...
            if (tipo == "AGRUPADO") {
              opcionCosto.listCostos
                ...
                .forEach((element) => {
                  ...
                  if (
                    element.code_cost == 4 &&
                    state.datosPrincipales.volumen < 1
                  ) {
                    if (factorCalculado < 1) {
                      factorCalculado = 1;
                    }
...
async function calcularTotalFleteVentaPorOpcion(data) {
  ...
  data
    ...
    .forEach(async (element) => {
      let factor = miMixin.methods.calcularFac(
        state.listMultiplicador.filter((v) => v.id == element.id_multiplicador)
          .length > 0
          ? state.listMultiplicador.filter(
              (v) => v.id == element.id_multiplicador,
            )[0].code
...
export function getNombreCotizacion(listNotasQuote) {
  ...
  let name = `COTIZACION ${nota} ${state.datosPrincipales.nombre}`;
  if (shipment.code == "FCL") {
    containers = state.datosPrincipales.containers.map((c) => c.code).join("_");
    name = `COTIZACION ${containers} FCL ${state.datosPrincipales.nombre}`;
  }
